...
const AttentionFlowGraph = () => {
  ...
  const drawGraph = React.useCallback(() => {
    ...
    const linkElements = g.selectAll("line")
      ...
      .attr("stroke-width", (d: Link) => 2 + d.weight * 8) // Increased base width and multiplier
      .attr("opacity", 0.5)
      .style("cursor", "pointer")
      .on("mouseover", function() {
        d3.select(this)
          .attr("opacity", 0.8)
          .attr("stroke-width", (d: Link) => (2 + d.weight * 8) * 1.5); // Increase width on hover
      })
      .on("mouseout", function() {
        d3.select(this)
          .attr("opacity", 0.5)
          .attr("stroke-width", (d: Link) => 2 + d.weight * 8);
      });
    ...
    g.selectAll("circle.hover-target")
      ...
      .on("mouseover", function(event, d) {
        ...
        d3.select(this.parentNode)
          .select("circle:not(.hover-target)")
          .attr("r", 14)
          .attr("fill", "#d1d5db");
      })
      .on("mouseout", function() {
    ...
    g.selectAll("line.hover-target")
      ...
      .on("mouseover", function(event, d) {
        ...
        tooltip.html(`Weight: ${d.weight.toFixed(4)}${group ? `<br>Group: ${group.name}` : '<br>Individual Head'}`);
        ...
        // Highlight edge
        const edge = d3.select(this.parentNode).select("line:not(.hover-target)");
        edge
          .attr("opacity", 0.8)
          .attr("stroke-width", (d: Link) => (2 + d.weight * 8) * 1.5);
      })
      .on("mouseout", function() {
        // Remove tooltip
        d3.select("body").selectAll(".tooltip").remove();
