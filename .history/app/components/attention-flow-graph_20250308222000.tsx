...
const AttentionFlowGraph = () => {
  ...
  const handleHeadSelection = (input: string) => {
    try {
      ...
      const [layer, head] = line.split(',').map(num => parseInt(num.trim()));
      if (isNaN(layer) || isNaN(head) || layer < 0 || head < 0 || layer >= data.numLayers || head >= data.numHeads) {
        throw new Error(`Invalid head pair: ${line}`);
      }

      // Only allow selection if head is not in any group
      if (getHeadGroup(layer, head) === null) {
        setSelectedHeads([{ layer, head }]); // Set only one head
      }
    } catch (error) {
      console.error('Error parsing head indices:', error);
  ...
  const handleNumHeadsChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    // Clear selected head when changing number of heads
    setSelectedHeads([]);
  };
  ...
  const drawGraph = React.useCallback(() => {
    ...
    const graphWidth = width - legendWidth; // Adjust main graph width
    const tokenWidth = (graphWidth - 2 * padding) / (data.numTokens);
    ...
    const legend = svg.append("g")
      .attr("transform", `translate(${graphWidth + padding}, ${padding})`);
        ...
        <>
          <div className="flex flex-col gap-2 p-4 border rounded">
            ...
            <div>
              ...
              <div className="space-y-4">
                <div>
                  <div className="text-sm text-gray-600 mb-2">Selected heads:</div>
                  <div className="flex flex-wrap gap-2">
                ...
                <div>
                  <div className="text-sm text-gray-600">
                    Add a head:<br/>
                    Format: layer,head<br/>
                    Example: 0,1
                  </div>
                  <textarea
                    className="w-full p-2 border rounded font-mono mt-2"
                    rows={1}
                    placeholder="0,1"
                    onChange={(e) => handleHeadSelection(e.target.value)}
                  />
                  <div className="text-sm text-gray-600">
                    Valid ranges: Layer (0-{data.numLayers - 1}), Head (0-{data.numHeads - 1})
                  </div>
                </div>
              </div>
            </div>
