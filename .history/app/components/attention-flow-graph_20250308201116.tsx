...
const AttentionFlowGraph = () => {
  const [data, setData] = useState<GraphData>({
    ...
    numHeads: 4,
    attentionPatterns: [],
    tokens: Array(5).fill('token')  // Default placeholder tokens
  });
  ...
  const addHeadToGroup = (head: number, groupId: number) => {
    setHeadGroups(prev => {
      ...
      if (targetGroup.heads.includes(head)) {
        return prev.map(group =>
          group.id === groupId
            ? { ...group, heads: group.heads.filter(h => h !== head) }
            : group
        );
      }
    ...
    const currentGroup = headGroups.find(g => g.heads.includes(head));
    if (currentGroup?.id === groupId) {
      setSelectedHeads(prev => [...prev, head]);
    } else {
  ...
  // Function to fetch attention data from an API
  const fetchAttentionData = async (text: string) => {
    try {
      ...
      const response = await fetch('/api/attention', {
        ...
        body: JSON.stringify({
          text,
          // Add any other parameters needed by your backend
        }),
      });
      ...
      alert('Failed to fetch attention data');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="flex flex-col gap-4 p-4">
                ...
                <button
                  ...
                  style={{
                    backgroundColor: isGrouped 
                      ? d3.schemeTableau10[groupId % 10]  // Group colors
                      : selectedHeads.includes(i)
                        ? d3.schemePaired[i % 12]  // Individual head colors
                        : undefined
                  }}
                  disabled={isGrouped}
                  title={isGrouped ? `Head ${i} is part of ${headGroups.find(g => g.id === groupId)?.name}` : undefined}
                >
                  Head {i}
                </button>
              );
      ...
      <div className="flex flex-col gap-2 p-4 border rounded">
        ...
        <textarea
          ...
          onChange={(e) => fetchAttentionData(e.target.value)}
        />
      </div>
