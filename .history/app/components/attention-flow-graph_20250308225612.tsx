...
const AttentionFlowGraph = () => {
  ...
  const drawGraph = React.useCallback(() => {
    ...
    for (let l = 0; l < data.numLayers; l++) {
      g.append("text")
        .attr("x", padding.left / 2 + 15)  // Moved numbers to the right
    ...
    // Y-axis label (Layers)
    g.append("text")
      .attr("x", padding.left / 2 - 25)  // Moved label to the left
      .attr("y", height / 2)
      .attr("text-anchor", "middle")
      .attr("dominant-baseline", "middle")
      .attr("transform", `rotate(-90, ${padding.left / 2 - 25}, ${height / 2})`)  // Updated rotation center
      .attr("font-size", "14px")
