...
const AttentionFlowGraph = () => {
  ...
  const handleHeadSelection = (input: string) => {
    try {
      // Parse the input text into head pairs
      ...
      const validPairs = pairs.filter(pair => getHeadGroup(pair.layer, pair.head) === null);
      setSelectedHeads(validPairs);
    } catch (error) {
